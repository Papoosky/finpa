...
export default function DashboardScreen({ token, onUnauthorized }: Props) {
  ...
  const pieData = useMemo(() => {
    ...
    const entries: { name: string; total: number }[] = top5.map(([name, total]) => ({
      name,
      total,
    }));
    if (otrosTotal > 0) {
          ...
          {transactions.length > 0 ? (
            <>
              ...
              {pieData.slices.length > 0 && (
                <>
                  <Text style={[styles.sectionTitle, { marginTop: 28 }]}>Gastos por categoria</Text>
                  <View style={styles.chartContainer}>
                    <VictoryPie
                      data={pieData.slices}
